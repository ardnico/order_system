...
- プロジェクト種別: Web アプリ (FastAPI + Jinja を使った家事受発注ボード)
- 主な目的:
  - 家事タスクを発注・受注し、完了承認時にポイントを付与する
  - ポイント残高や履歴、ご褒美（リワード）申請・承認を家庭内で共有する
- 想定ユーザー:
  - 同居する家庭メンバー（夫婦・カップル・親子など）が PC/スマホブラウザで利用
- 非ゴール:
  - モバイルネイティブアプリの提供
  - 外部サービス連携や高度な分析ダッシュボード
  - 法人向けの複雑な多段承認フロー
...
- 言語: Python
- フレームワーク・ライブラリ:
  - フロント: サーバーサイド HTML (Jinja2 テンプレート + シンプル CSS)
  - バックエンド: FastAPI
  - DB/ORM: SQLite + SQLModel (SQLAlchemy ベース)
- インフラ: ローカル実行前提（開発サーバーは `uvicorn`）
- バージョン: `requirements.txt` に記載されたバージョンに従うこと

**禁止事項:**

- 外部サービス連携を勝手に追加しない
- メンテされていない／脈絡のないライブラリ導入や巨大スニペットの持ち込み
