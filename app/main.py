...
ROOT_EMAIL = os.getenv("ROOT_EMAIL", "root@local")
ROOT_PASSWORD = os.getenv("ROOT_PASSWORD", "rootpass")
ROOT_HOUSEHOLD_NAME = os.getenv("ROOT_HOUSEHOLD", "Root Household")
ROOT_JOIN_CODE = os.getenv("ROOT_JOIN_CODE", "rootadmin")


def ensure_root_admin():
    with Session(engine) as session:
        household = session.exec(
            select(Household).where(Household.name == ROOT_HOUSEHOLD_NAME)
        ).first()
        if not household:
            household = Household(
                name=ROOT_HOUSEHOLD_NAME,
        ...
        user = session.exec(select(User).where(User.email == ROOT_EMAIL)).first()
        if not user:
            user = User(
...
UI_STRINGS = {
    "en": {
        ...
        "menus.image": "Menu image",
        "menus.addRow": "Add ingredient row",
        ...
        "settings.contributionRate": "Contribution rate (actions per point)",
        ...
        "admin.users": "Users",
        "admin.createHousehold": "Create household",
        "admin.createUser": "Create user",
        "admin.edit": "Edit",
        "admin.delete": "Delete",
        "admin.update": "Update",
        "admin.passwordOptional": "Password (blank = keep)",
        "admin.adminFlag": "Administrator",
        "admin.joinCode": "Join code",
        "admin.language": "Language",
        "admin.theme": "Theme",
        "admin.font": "Font",
        "admin.rate": "Contribution rate",
    ...
    "ja": {
        ...
        "settings.contributionRate": "貢献ポイントのレート（何回で1pt）",
        ...
        "admin.heading": "管理者画面",
        "admin.households": "世帯一覧",
        "admin.users": "ユーザー一覧",
        "admin.createHousehold": "世帯を追加",
        "admin.createUser": "ユーザーを追加",
        "admin.edit": "編集",
        "admin.delete": "削除",
        "admin.update": "更新",
        "admin.passwordOptional": "パスワード（空欄なら変更なし）",
        "admin.adminFlag": "管理者権限",
        "admin.joinCode": "参加コード",
        "admin.language": "言語",
        "admin.theme": "テーマ",
        "admin.font": "フォント",
        "admin.rate": "貢献レート",
...
async def store_menu_image(file: Optional[UploadFile]) -> Optional[str]:
    if not file or not file.filename:
        return None
    ext = os.path.splitext(file.filename)[1].lower() or ".png"
    if ext not in ALLOWED_IMAGE_EXTENSIONS:
        ext = ".png"
    filename = f"menu-{secrets.token_hex(8)}{ext}"
    path = os.path.join(uploads_dir, filename)
    content = await file.read()
    with open(path, "wb") as f:
        f.write(content)
...
def award_contribution_points(
    session: Session, user: User, household: Household, description: str
):
    rate = household.contribution_rate or 10
    user.contribution_credit = (user.contribution_credit or 0) + 1
    earned = user.contribution_credit // rate if rate > 0 else 0
    user.contribution_credit = user.contribution_credit % rate if rate > 0 else 0
    session.add(user)
    if earned:
        tx = PointTransaction(
            household_id=household.id,
            ...
            description=f"Contribution: {description}",
        )
        session.add(tx)
...
def export_household_data(session: Session, household_id: int) -> dict:
    ...
    return {
        ...
        "household": {
            ...
            "language": household.language if household else None,
            "theme": household.theme if household else None,
            "font": household.font if household else None,
            "contribution_rate": household.contribution_rate if household else 10,
        },
        "meal_sets": [
            {
                "name": s.name,
...
def import_household_data(session: Session, household_id: int, payload: dict):
    ...
    household = session.get(Household, household_id)
    if household and payload.get("household"):
        info = payload.get("household", {})
        household.language = info.get("language") or household.language
        household.theme = info.get("theme") or household.theme
        household.font = info.get("font") or household.font
        try:
            household.contribution_rate = max(1, int(info.get("contribution_rate", household.contribution_rate or 10)))
    ...
    for menu_entry in payload.get("menus", []):
        ...
        if not menu:
            menu = Menu(
                ...
                dish_type_id=dish_type_id,
                image_url=menu_entry.get("image_url"),
            )
            session.add(menu)
            session.commit()
            session.refresh(menu)
            ...
            menu.image_url = menu_entry.get("image_url") or menu.image_url
            session.add(menu)
            session.commit()
...
def save_menu_ingredients(
    ...
    for name, qty_raw, unit in zip(names, quantities, units):
        ...
        if isinstance(qty_raw, list):
            qty_raw = qty_raw[0] if qty_raw else ""
        ...
        if not unit_option and ingredient.unit:
            unit_option = unit_lookup.get(ingredient.unit)
        session.add(
            MenuIngredient(
                menu_id=menu.id,
            ...
            if isinstance(entry, str) and entry.strip().startswith("["):
                try:
                    ...
                    if isinstance(parsed, list):
                        flattened.extend([str(v) for v in parsed])
                        continue
                except Exception:
                    pass
    ...
    return templates.TemplateResponse(
        ...
        build_context(
            ...
            user,
            {
                "households": households,
                "users": users,
                "root_household_name": ROOT_HOUSEHOLD_NAME,
...
async def admin_create_household(
    ...
    household = Household(
        name=cleaned,
        join_code=code,
        language=language if language in UI_STRINGS else "ja",
        theme=theme if theme in THEME_CHOICES else THEME_CHOICES[0],
        font=font if font in FONT_CHOICES else FONT_CHOICES[0],
        contribution_rate=max(1, int(contribution_rate)) if contribution_rate else 10,
    )
    session.add(household)
    session.commit()
    flash(request, "Household created")
    return RedirectResponse("/admin", status_code=303)


@app.post("/admin/households/{household_id}/update")
async def admin_update_household(
    ...
    if not household:
        flash(request, "Household not found", "error")
        return RedirectResponse("/admin", status_code=303)
    household.name = name.strip() or household.name
    if join_code is not None:
        household.join_code = join_code.strip().lower() or household.join_code
    household.language = language if language in UI_STRINGS else household.language
    ...
    try:
        household.contribution_rate = max(1, int(contribution_rate))
    except (TypeError, ValueError):
        pass
    session.add(household)
    session.commit()
    flash(request, "Household updated")
    return RedirectResponse("/admin", status_code=303)


@app.post("/admin/households/{household_id}/delete")
async def admin_delete_household(
    ...
    household = session.get(Household, household_id)
    if not household or household.name == ROOT_HOUSEHOLD_NAME:
        flash(request, "Household not found", "error")
        return RedirectResponse("/admin", status_code=303)
    for member in session.exec(select(User).where(User.household_id == household.id)).all():
        session.delete(member)
    session.delete(household)
    session.commit()
    flash(request, "Household deleted")
    return RedirectResponse("/admin", status_code=303)
...
async def admin_create_user(
    ...
    if not household:
        ...
        return RedirectResponse("/admin", status_code=303)
    existing = session.exec(
        select(User).where(User.household_id == household_id, User.email == email)
    ).first()
    if existing:
        flash(request, "User already exists", "error")
        return RedirectResponse("/admin", status_code=303)
...
async def admin_update_user(
    ...
    if not target or target.email == ROOT_EMAIL:
        flash(request, "User not found", "error")
        return RedirectResponse("/admin", status_code=303)
    target.display_name = display_name.strip() or target.display_name
    target.email = email
    target.household_id = household_id
...
async def update_language(
    ...
    if household:
        ...
        try:
            ...
            household.contribution_rate = household.contribution_rate or 10
...
async def create_menu(
    ...
    ingredient_names = ingredient_names or form_data.getlist("ingredient_names")
    ingredient_quantities = ingredient_quantities or form_data.getlist("ingredient_quantities")
    ingredient_units = ingredient_units or form_data.getlist("ingredient_units")
    cleaned_name = name.strip()
    if not cleaned_name:
        flash(request, "Menu name required", "error")
        return RedirectResponse("/menus", status_code=303)
    ingredient_names = normalize_list_field(ingredient_names)
    ingredient_quantities = normalize_list_field(ingredient_quantities)
    ingredient_units = normalize_list_field(ingredient_units)
    ...
    session.commit()
    session.refresh(menu)
    image_url = await store_menu_image(image_file)
    if image_url:
    ...
    award_contribution_points(session, user, household, f"Menu {menu.name}")
    session.commit()
...
async def create_meal_plan(
    ...
    ensure_meal_plan_days(session, plan)
    if household:
        award_contribution_points(session, user, household, f"Meal plan {plan.name}")
        session.commit()
...
async def update_meal_plan(
    ...
    ensure_meal_plan_days(session, plan)
    form_data = await request.form()
    day_dates = normalize_list_field(day_dates or form_data.getlist("day_dates"))
    lunch_menu_ids = normalize_list_field(lunch_menu_ids or form_data.getlist("lunch_menu_ids"))
    dinner_menu_ids = normalize_list_field(dinner_menu_ids or form_data.getlist("dinner_menu_ids"))
    lunch_set_ids = normalize_list_field(lunch_set_ids or form_data.getlist("lunch_set_ids"))
    dinner_set_ids = normalize_list_field(dinner_set_ids or form_data.getlist("dinner_set_ids"))
    days = session.exec(
        select(MealPlanDay).where(MealPlanDay.meal_plan_id == plan.id)
    ).all()
    ...
    for idx, (day_str, lunch_id, dinner_id) in enumerate(
        ...
        for slot, set_val in (
            ...
            for req in requirement_map[set_val]:
                ...
                values = normalize_list_field(form_data.getlist(key))
